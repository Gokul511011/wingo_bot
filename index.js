...
function masterGuidePatternEngine(history) {
    ...
    try {
        ...
        let allResults = allNumbers.map(n =>
            n >= 5 ? "BIG" : "SMALL"
        );
        ...
        */
            ...
            patternName =
                `⚡ Quadra/Triple Trend (${streak}x Streak)`;
        ...
        */
            ...
            patternName =
                "⚖️ 2-in-1 / 3-in-1 Correction Phase";
        ...
        */
        ...
        let scanLimit =
            Math.min(history.length - 1, 300);

        for (let i = 0; i < scanLimit; i++) {

            let currN =
                parseInt(
                    history[i].number !== undefined
                        ? history[i].number
                        : history[i].result
                );
        ...
        for (let n = 0; n <= 9; n++) {
            ...
            if (
                (predResult === "BIG" && isBig) ||
                (predResult === "SMALL" && !isBig)
            ) {

                candidates.push({
                    num: n,
                    count: numFrequency[n]
                });
        ...
        */
        ...
        if (
            lastNum === 7 &&
            matchedNumbers.includes(0) &&
            matchedNumbers.includes(6)
        ) {

            patternName =
                "🎯 Guide Rule: After [7] -> [0, 6]";
        }
...
app.get(
    '/',
    (req, res) =>
        res.send('Master Guide Pattern Engine Active!')
);
...
async function fetchWinGoData() {
    ...
    try {
        ...
        */
        ...
        let nextPeriod =
            String(
                BigInt(actualPeriod) + 1n
            );


        /*
        =================================================
        6–7 SECOND ADVANCE TIMING
        =================================================

        Current 30-second round:

        23 sec -> about 7 sec remaining
        24 sec -> about 6 sec remaining

        So prediction is sent only during 23–24 sec.
        =================================================
        */
        ...
        */
        ...
        if (
            secondsIntoPeriod < 23 ||
            secondsIntoPeriod > 24
        ) {
            return;
        }
        ...
        */
        ...
        if (
            ...
            let isResultHit =
                lastPredictedResult === actualResult;


            let isNumberHit =
                lastPredictedNumbers.includes(
                    actualNum
                );
            ...
            if (isResultHit) {
                ...
                */
                ...
                if (isNumberHit) {
                    ...
                    dynamicStatusMsg =
                        `🎉 **CONGRATULATIONS (LEVEL ${maintenanceLevel} ₹${winAmount} WIN + JK)** 🎉\n` +
                    ...
                    dynamicStatusMsg =
                        `🎉 **CONGRATULATIONS (LEVEL ${maintenanceLevel} ₹${winAmount} WIN)** 🎉\n` +
            ...
            */
                ...
                */
                ...
                if (
                    ...
                    dynamicStatusMsg =
                        `💔 **LOSS AT LEVEL 8: ${actualResult} (${actualNum})**\n` +
                    ...
                    dynamicStatusMsg =
                        `💔 **LOSS: ${actualResult} (${actualNum} - ${actualColor})**\n` +
...
app.listen(
    ...
    () => {
        ...
        console.log(
            "Master Guide Bot Active on port " +
            PORT
        );
